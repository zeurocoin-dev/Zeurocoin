...
bool CSporkManager::CheckSignature(CSporkMessage& spork)
{
    ...
    if(!nitroSendSigner.VerifyMessage(pubkey, spork.vchSig, strMessage, errorMessage)){
        return false;
    }
...
bool CSporkManager::Sign(CSporkMessage& spork)
{
    ...
    if(!nitroSendSigner.SetKey(strMasterPrivKey, errorMessage, key2, pubkey2))
    {
        LogPrintf("CMasternodePayments::Sign - ERROR: Invalid masternodeprivkey: '%s'\n", errorMessage);
        return false;
    }

    if(!nitroSendSigner.SignMessage(strMessage, errorMessage, spork.vchSig, key2)) {
        LogPrintf("CMasternodePayments::Sign - Sign message failed");
        return false;
    }

    if(!nitroSendSigner.VerifyMessage(pubkey2, spork.vchSig, strMessage, errorMessage)) {
