...
CAmount AmountFromValue(const Value& value)
{
    ...
    if (dAmount <= 0.0 || dAmount > 900000000.0)
...
{ //  category              name                      actor (function)         okSafeMode threadSafe reqWallet
  //  --------------------- ------------------------  -----------------------  ---------- ---------- ---------
    ...
    /* Aspire features */
    { "zeuro",               "masternode",             &masternode,             true,      true,       false },
    { "zeuro",               "masternodelist",         &masternodelist,         true,      true,       false },
    { "zeuro",               "mnbudget",               &mnbudget,               true,      true,       false },
    { "zeuro",               "mnbudgetvoteraw",        &mnbudgetvoteraw,        true,      true,       false },
    { "zeuro",               "mnfinalbudget",          &mnfinalbudget,          true,      true,       false },
    { "zeuro",               "mnsync",                 &mnsync,                 true,      true,       false },
    { "zeuro",               "spork",                  &spork,                  true,      true,       false },
#ifdef ENABLE_WALLET
    { "zeuro",               "nitrosend",               &nitrosend,               false,     false,      true  }, /* not threadSafe because of SendMoney */
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("To use zeurod, or the -server option to zeuro-qt, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
...
std::string HelpExampleCli(string methodname, string args){
    return "> zeuro-cli " + methodname + " " + args + "\n";
}

std::string HelpExampleRpc(string methodname, string args){
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
        "\"method\": \"" + methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:2627/\n";
}
