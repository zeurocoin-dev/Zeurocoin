...
//    bool SelectCoins(int64_t nTargetValue, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64_t& nValueRet, const CCoinControl *coinControl = NULL, AvailableCoinsType coin_type=ALL_COINS, bool useIX = true) const;
    bool SelectCoinsDark(int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& setCoinsRet, int64_t& nValueRet, int nNitrosendRoundsMin, int nNitrosendRoundsMax) const;
    bool SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, int64_t& nValueRet, int nNitrosendRoundsMin, int nNitrosendRoundsMax);
    ...
    std::string PrepareNitrosendDenominate(int minRounds, int maxRounds);
    int GenerateNitrosendOutputs(int nTotalValue, std::vector<CTxOut>& vout);
    ...
    // get the Nitrosend chain depth for a given input
    int GetRealInputNitrosendRounds(CTxIn in, int rounds) const;
    // respect current settings
    int GetInputNitrosendRounds(CTxIn in) const;
...
public:
    ...
    CAmount GetAnonymizableCredit(bool fUseCache=true) const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            ...
            const int rounds = pwallet->GetInputNitrosendRounds(vin);
            if(rounds >=-2 && rounds < nNitrosendRounds) {
                nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
                if (!MoneyRange(nCredit))
...
public:
    ...
    //Used with Nitrosend. Will return largest nondenom, then denominations, then very small inputs
    int Priority() const
    {
