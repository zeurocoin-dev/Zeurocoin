...
- [Building Aspire](#building-zeuro)
...
Clone the git repositories for zeuro and gitian and then checkout the zeuro version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the zeuro repository
...
gitian builds' in [doc/release-process.md](release-process.md) in the zeuro repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/zeuro/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/zeurocoin/zeuro
...
For example:
```bash
URL=https://github.com/crowning-/zeuro.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit zeuro=${COMMIT} --url zeuro=${URL} ../zeuro/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit zeuro=${COMMIT} --url zeuro=${URL} ../zeuro/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit zeuro=${COMMIT} --url zeuro=${URL} ../zeuro/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/zeuro-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/zeuro-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/zeuro-build.assert
```
...
[zeuro/gitian.sigs](https://github.com/zeurocoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
