...
- saction for Nitrosend. These are special transactions that require a signature that only the masternodes can create.
- Nitrosend now has no fees to track what-so-ever, all that will ever be in Nitrosend transactions are Nitrosend denominations.
...
- Split up nitrosend.cpp into masternode.cpp, activemasternode.cpp and instantx.cpp
- Added modular ProcessMessages for Nitrosend, Masternodes and InstantX
...
- Fixed all sorts of issues with collateral and Nitrosend.
...
- Nitrosend tested with 2.5 DRK successfully and I've set that as the minimum
...
- Nitrosend UI is disabled for masternodes now and titlebar says "[masternode]"
...
- Updated wording for nitrosend messages
- Removed disable nitrosend checkbox from config screen
- Added "-enablenitrosend" cli option
...
- Nitrosend should anonymize very close to the intended amount now
- Added Amount/Rounds to overview screen so you can see current settings
- Overview nitrosend cache is cleared on settings change (will instantly update)
...
- This update deals with freezing, slow wallets, slow load times and the
  ...
  Nitrosend process has been reserve keys for every attempt! To rectify this
...
- Easy to read nitrosend progress bar (Mouse over for very detailed info)
...
- Client now creates fee sizes inputs for use in nitrosend denomination phases
...
- Client now prioritises sending denominated funds back into Nitrosend. This will
...
- Nitrosend now checks diskspace before sending anything to a masternode.
...
- Majorly improved the way nitrosend participants are paired together. It should
  be super fast now.
...
- Nitrosend now shows address instead of "n/a"
...
- Added GUI configuration for Nitrosend Rounds, Enable Nitrosend and Anonymize
  amount of DRK
...
- Nitrosend limited to 5000DRK per wallet.dat. Client will warn about this the
  first time it's opened, then disable nitrosend from then on.
...
- Found race condition with new blocks and clearing nitrosend entries that was
...
- Nitrosend Denominate Outputs are now in a random order:
...
- Nitrosend now uses 3 participants instead of two.
- SplitUpMoney can now make collateral inputs when needed
- Transactions now shows nitrosend transaction types for easier understanding
  of what's going on:
...
- Fixed the money destruction bug, it was caused by "nitrosend denominate 8000".
  I missed a reference and the client passed an empty address to
  SendMoneyToDestination. rcp nitrosend source: http://pastebin.com/r14piKuq
...
- Added Nitrosend detection to the UI
- fixed senttoaddress, it will use all inputs when nitrosend is disabled now.
  Otherwise it will ONLY use non-denom.
- "nitrosend addr amount" now returns the hash of the transaction it creates
...
- Removed "nitrosend denominate", nitrosend now will figure out the most it can
  denominate. Use "nitrosend auto" instead.
...
- QT GUI will now ask to unlock the wallet when it detects Nitrosend wants to do
  something and lock it when it's done again.
- Nitrosend is turned off by default in the daemon now. In most cases daemons
  won't want to run with anonymity (pools, exchanges, etc), if a user does they
  can override the default setting with -enabledaemonnitrosend=true
- Fees per round of Nitrosend are 0.001DRK or $0.00538 at current prices. This
...
- RPC calls are changed a bit:
    ...
    nitrosend denominate 100 - Will denominate 100DRK
    nitrosend auto - Will run AutoDenominate
    nitrosend Xaddr 100 - Will send 100 denominated DRK to Xaddr
...
- Fixed nitrosend+ rounds function, it was returning bad data in some cases
  causing nitrosend inputs to never stop being sent through nitrosend.
...
/darkcoin-qt -datadir=/home/user/.darkcoin -listen=0 -nitrosendrounds=8 -usemasternode="192.168.56.102:16180"
/darkcoin-qt -datadir=/home/user/.darkcoin2 -listen=0 -nitrosendrounds=8 -usemasternode="192.168.56.102:16180"

and even disable nitrosend auto-denom if wanted:
/darkcoin-qt -datadir=/home/user/.darkcoin -listen=0 -nitrosendrounds=8 -usemasternode="192.168.56.102:16180" -disablenitrosend=1
...
- Added a smart input splitting method. Place 1000+DRK into a brand new wallet
  and it will be split into many inputs compatible with Nitrosend
...
- NitroSendRounds had a bug that I fixed, should calculate correctly now
...
- New column "Nitrosend Rounds" in coincontrol to show how secure a given input is
- Fixed a few issues causing nitrosend to fail. We should see many more nitrosends
...
- Redid denominations to 1, 10, 100, and 500. Maybe this is too simple, but it
  ...
  again through nitrosend for the next transactions. We'll see how it works.
...
- Improved NitrosendInput add entry verification. Masternodes will now reject
